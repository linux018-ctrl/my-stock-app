...
# --- 網頁設定 ---
st.set_page_config(page_title="台股多頭獵人 V5.3", layout="wide")
st.title("📈 台股多頭獵人 V5.3 - 中文名稱優化版")
...
selected_code = st.sidebar.selectbox("選擇個股", options=list(st.session_state.watchlist.keys()), format_func=lambda x: f"{x} {st.session_state.watchlist[x]}")
...
def get_fundamentals(stock_obj):
    try:
        ...
        if div_yield:
            div_yield_str = f"{round(div_yield, 2)}%" if div_yield > 1 else f"{round(div_yield * 100, 2)}%"
        else:
            div_yield_str = "N/A"
...
with tab1:
    if selected_code:
        ...
        data, ticker_obj = get_stock_data(selected_code, lookback_days + 150)
        ...
        if not data.empty:
            ...
            st.subheader(f"{stock_name} ({selected_code}) 技術分析")
            ...
            fig.add_trace(go.Bar(x=df_view.index, y=df_view[macd_col], marker_color=colors, name='MACD'), row=2, col=1)
...
with tab2:
    ...
    if st.button("🚀 掃描觀察名單"):
        ...
        for i, (code, name) in enumerate(stocks_list):
            df_scan, _ = get_stock_data(code, 100)
        ...
        if scan_results:
            ...
            st.dataframe(res_df.style.applymap(lambda x: 'color: red' if isinstance(x, float) and x > 0 else 'color: green' if isinstance(x, float) and x < 0 else '', subset=['漲幅%']), 
...
with tab3:
    ...
    st.write("請選擇您感興趣的「概念」或「集團」，程式將自動掃描其中的轉折訊號。")
    ...
    if st.button("🎯 開始掃描"):
        ...
        for i, code in enumerate(scan_list):
            df_s, _ = get_stock_data(code, 120)
            if not df_s.empty:
                try:
                    ...
                    if is_kd_cross: 
                        score += 1
                        reasons.append("KD低檔金叉")
                    if is_break_ma60: 
                        score += 1
                        reasons.append("突破季線")
                    if is_above_ma20 and is_macd_turning:
                        score += 1
                        reasons.append("站穩月線+動能")

                    if score >= 1:
                        # --- 關鍵修正：優先從中文對照表抓名字 ---
                        # 邏輯：先查使用者名單 -> 再查內建對照表 -> 最後才顯示代號
                        name = st.session_state.watchlist.get(code, STOCK_NAMES.get(code, code))
                        
                        reversal_stocks.append({
                            "代號": code, 
                            "名稱": name, # 現在這裡會正確顯示 "光聖" 或 "智邦"
                            "收盤價": curr['Close'],
        ...
        if reversal_stocks:
            st.success(f"在「{target_sector}」中發現 {len(reversal_stocks)} 檔潛在轉折股！")
            rev_df = pd.DataFrame(reversal_stocks)
            st.dataframe(rev_df, column_config={"收盤價": st.column_config.NumberColumn(format="%.2f")}, use_container_width=True)
        else:
